...
@Component({
  selector: 'app-live-monitor-full',
  imports: [CommonModule, TranslateModule, ButtonModule],
  templateUrl: './live-monitor-full.component.html',
  styleUrls: ['./live-monitor-full.component.css']
})
export class LiveMonitorFullComponent implements OnDestroy, AfterViewInit {
  ...
  errorMessage = '';
  autoRefresh = true;
  private refreshIntervalMs = LiveMonitorFullComponent.DEFAULT_REFRESH_MS;
  ...
  loadLiveMatches(): void {
    ...
    this.liveMatchesSubscription = this.supabaseService
      ...
      .pipe(
        ...
        catchError((error) => {
          if (error?.status === 429) {
            ...
            this.setRefreshInterval(LiveMonitorFullComponent.RATE_LIMIT_REFRESH_MS);
            return of([] as Match[]);
          }
      ...
      .subscribe((matches) => {
        ...
        this.ngZone.run(() => {
          this.setRefreshInterval(LiveMonitorFullComponent.DEFAULT_REFRESH_MS);
